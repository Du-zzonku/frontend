...
export function StudyLeftPanel({
  ...
  return (
    <aside className="h-full flex">
      ...
      {/* Content Panel */}
      <div className="w-[260px] bg-[#0b1022]/90 border-r border-border flex flex-col overflow-hidden">
        {/* Back + Title */}
        <div className="p-4 pb-0">
          ...
          <h1 className="text-xl font-bold text-foreground">{model.nameKo}</h1>
        </div>
        ...
        <div className="px-4 mt-3">
          <div className="flex items-center gap-4 text-sm">
            ...
            <span className="text-foreground">기계공학</span>
          </div>
        </div>
        ...
        <div className="px-4 mt-4">
          ...
          <div className="flex flex-wrap gap-1.5">
            {keywords.map((keyword, index) => (
              <span
                key={index}
                className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs bg-primary/15 text-primary border border-primary/30"
              >
                {keyword}
                <button
        ...
        {/* Memo */}
        <div className="px-4 mt-5 flex-1 flex flex-col min-h-0">
          <p className="text-sm font-medium text-foreground mb-2">메모</p>
          <textarea
            value={notes}
            onChange={(e) => onNotesChange(e.target.value)}
            placeholder="학습 내용을 메모로 남겨 보세요."
            className="flex-1 w-full p-3 text-sm bg-transparent rounded-lg border border-border/50 focus:border-primary/50 focus:outline-none resize-none transition-colors text-foreground placeholder:text-muted-foreground/50"
          />
        </div>

        {/* PDF Download Button */}
        <div className="p-4">
          <button className="w-full py-3 px-4 rounded-xl bg-primary hover:bg-primary/90 text-primary-foreground text-sm font-medium transition-colors flex items-center justify-center gap-2">
